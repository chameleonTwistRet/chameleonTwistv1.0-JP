...
void _bcopy(char* destination, char* src, s32 size);
...
static OSTask* _VirtualToPhysicalTask(OSTask* intp) {
    OSTask* tp;
    tp = &tmp_task;
    _bcopy((char*)intp, (char*)tp, sizeof(OSTask));
...
void osSpTaskLoad(OSTask* intp) {
    ...
    while (__osSpSetPc((s32)SP_IMEM_START) == -1) {
        ;
    }
    while (__osSpRawStartDma(OS_WRITE, (s32)(SP_IMEM_START - sizeof(*tp)), tp, sizeof(OSTask)) == -1) {
        ;
    }
    while (__osSpDeviceBusy()) {
        ;
    }
    while (__osSpRawStartDma(OS_WRITE, (s32)SP_IMEM_START, tp->t.ucode_boot, tp->t.ucode_boot_size) == -1) {
        ;
    }
}
