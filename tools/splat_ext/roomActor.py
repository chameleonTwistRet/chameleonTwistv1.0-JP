...
class N64SegRoomActor(CTSegBase):
    SUFFIX = "roomAct"
    ...
    def disassemble_data(self, rom_bytes):
        ...
        if len(buf) != 96:
            error(
                f"Error: roomAct segment {self.name} length ({len(buf)}) is not valid"
            )

        sym = self.retrieve_sym_type(
            symbols.all_symbols_dict, self.vram_start, "Roomact"
        ) or self.create_symbol(
            addr=self.vram_start, in_segment=True, type="Roomact", define=True
        )

        data = struct.unpack(">iffffffifffiffffiiiiiiii", buf)

        lines = []
        if not self.data_only:
            lines.append('#include "common.h"')
            lines.append("")
            lines.append(f"RoomActor {sym.name} = {{")
